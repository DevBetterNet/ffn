...
public interface ILocalizationService
{
    ...
    /// The task result contains the locale string resource
    /// </returns>
    Task<LocaleStringResource> GetLocaleStringResourceByNameAsync(string resourceName, Guid languageId,
    ...
    /// </summary>
    /// <param name="languageId">Language identifier</param>
    /// <param name="loadPublicLocales">A value indicating whether to load data for the public webApp only (if "false", then for admin area only. If null, then load all locales. We use it for performance optimization of the site startup</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    ...
    /// The task result contains a string representing the requested resource string.
    /// </returns>
    Task<string> GetResourceAsync(string resourceKey, Guid languageId,
    ...
    Task<TPropType> GetLocalizedAsync<TEntity, TPropType>(TEntity entity, Expression<Func<TEntity, TPropType>> keySelector,
        Guid languageId = default, bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
    ...
    Task<string> GetLocalizedSettingAsync<TSettings>(TSettings settings, Expression<Func<TSettings, string>> keySelector,
        Guid languageId, Guid webAppId, bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
    ...
    /// </returns>
    Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, Guid languageId = default) where TEnum : struct;
    ...
    /// The task result contains the localized value
    /// </returns>
    Task<string> GetLocalizedPermissionNameAsync(PermissionRecord permissionRecord, Guid languageId = default);
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    Task AddLocaleResourceAsync(IDictionary<string, string> resources, Guid languageId = default);
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    Task DeleteLocaleResourcesAsync(IList<string> resourceNames, Guid languageId = default);
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    Task DeleteLocaleResourcesAsync(string resourceNamePrefix, Guid languageId = default);
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    Task SaveLocalizedFriendlyNameAsync<TPlugin>(TPlugin plugin, Guid languageId, string localizedFriendlyName)
        where TPlugin : IPlugin;
}
